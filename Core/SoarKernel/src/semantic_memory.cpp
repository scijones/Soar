...
void smem_lti_activation_history(agent *thisAgent, smem_lti_id lti, wma_decay_element* wma_decay_el)
{
    if (lti == NIL) return;
    ...
    for ( int i=0; i<WMA_DECAY_HISTORY; i++ )
    {
        if (i < n)
        {
            wma_decay_el->touches.access_history[i].d_cycle = static_cast<int>(thisAgent->smem_stmts->history_get->column_int(i));
...
inline double smem_lti_activate(agent* thisAgent, smem_lti_id lti, bool add_access, uint64_t num_edges = SMEM_ACT_MAX, wme* wme_p = NULL)
{
    ...
    else if (act_mode == smem_param_container::act_base)
    {
        if (prev_access_n == 0)
        {
            if (add_access)
            {
                // If we pass a working memory decay element
                if (wme_p && wme_p->wma_decay_el)
                {
                    // then we should import the history from that decay element.
                    wma_decay_element* temp_el = wme_p->wma_decay_el;
